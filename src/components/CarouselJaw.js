    ...
    render() {
        return (
            <div className="carousel__jaw">
                <span onClick={this.props.onClickClose} className="button__x" />
                ...
                <div className="carousel__jaw-rating">
                    {this.state.movieInfo.Ratings && (
                        <Ratings ratings={this.state.movieInfo.Ratings} />
                    )}
                    <span>
                        <strong>Runtime:</strong> {this.state.movieInfo.Runtime}
                    </span>
                    <span>
                        <strong>Genre:</strong> {this.state.movieInfo.Genre}
                    </span>
                    <span>
                    ...
                    <span>
                        <strong>Cast:</strong> {this.state.movieInfo.Actors}
                    </span>
                    <span>
                        <strong>Awards:</strong> {this.state.movieInfo.Awards}
                    </span>
                </div>
...
function Ratings(props) {
    ...
    return <ul>{listRatings}</ul>;
}

function ListRating(props) {
    const convertToPercentage =
        props.rate.Source === "Internet Movie Database"
            ? parseFloat(props.rate.Value).toFixed(2) / 10
            : parseInt(props.rate.Value) / 100;
    const rateSource =
        props.rate.Source === "Internet Movie Database"
            ? "IMDB"
            : props.rate.Source;
