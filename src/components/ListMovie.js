...
class MovieAppList extends React.Component {
	...
	getAvatar (uid) {
		([].concat(uid)).forEach(eachUID => {
			firebaseConfig.database.ref("/users/" + eachUID)
				.once("value")
				.then((snapshot) => {
					if(snapshot.val() !== null) {
						this.setState({
							imageURLs: this.state.imageURLs.concat(snapshot.val().photoURL)
						})
					}
				})
		});
	...
	render() {
		...
		const UpvoteButton = <button className="button__upvote" onClick={() => this.props.upvoteMovie({movie}, index)}><svg xmlns="http://www.w3.org/2000/svg" viewBox="100 -30 190 150">
		...
		const RemoveButton = <button className="button__delete" onClick={() => this.props.removeMovie(index)}>&times;</button>;
		const likedUsers = movie.stars ? <span className="MovieApp__result-list-avatars">{ pics }</span> : <span className="MovieApp__result-list-avatars">empty</span>;
...
function _MovieAppList (props) {
	...
	const likedUsers = movie.stars ? <span className="MovieApp__result-list-avatars">{ getAvatar(Object.keys(movie.stars), avatar) }</span> : <span className="MovieApp__result-list-avatars">empty</span>;
