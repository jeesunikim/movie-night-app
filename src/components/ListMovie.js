...
const ListMovie = props => {
  const moviesArr = Object.keys(props.movies);
      ...
      <ul className="MovieApp__result-list">
        {moviesArr.map(key => (
          <MovieAppList
            ...
            index={key}
            movies={props.movies}
            movie={props.movies[key]}
            removeMovie={props.removeMovie}
            upvoteMovie={props.upvoteMovie}
          />
        ))}
      </ul>
    </div>
...
function _MovieAppList(props) {
  ...
  const RemoveButton = (
    <button className="button__delete" onClick={() => props.removeMovie(index)}>
      &times;
    </button>
  );
  const likedUsers = movie.stars ? (
